...
CACHE_FILE = "/app/config/meteocat_cache.json"
...
class MeteocatService(StdService):
    ...
    def _fetch_and_cache(self):
        if not self.api_key or not self.city_code or self.api_key == "EL_TEU_API_KEY":
            weewx.log.logerr("[Meteocat] Falta configurar una api_key valida o city_code a weewx.conf")
            return
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as response:
                if response.status == 200:
                    ...
                    weewx.log.loginfo("[Meteocat] Cache actualitzada amb exit al disc.")
